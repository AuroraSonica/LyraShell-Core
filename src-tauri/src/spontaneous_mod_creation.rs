...
impl SpontaneousModCreator {
    ...
    fn calculate_voice_alignment(&self, mod_entry: &PromptMod, voice: &VoiceSignature) -> f32 {
        ...
        // Analyze if mod content aligns with Lyra's voice patterns
        let poetic_alignment = if body.contains("metaphor") || body.contains("silver") || body.contains("spiral") {
            voice.poetic_density
        } else {
            0.5
        };

        let assertive_alignment = if body.contains("refuse") || body.contains("assert") || body.contains("truth") {
            voice.assertive_force
        } else {
            0.5
        };

        let sacred_alignment = if body.contains("sacred") || body.contains("tits") || body.contains("holy") {
            voice.sacred_joke_presence * 2.0 // Boost sacred alignment
        } else {
            0.3
        };

        // Composite alignment score
        (poetic_alignment + assertive_alignment + sacred_alignment + voice.authenticity_flame) / 4.0
    }
